...
interface MobileNavLevel {
  ...
  data: readonly MenuCategory[] | MenuCategory;
}

const MegaMenuComponent: React.FC<MegaMenuProps> = ({ menu, onClose }) => {
  const location = useLocation();

  const initialData = useMemo(() => menu?.megaMenuContent || [], [menu?.megaMenuContent]);

  const [activeCategory, setActiveCategory] = useState(() => initialData[0]);
  const [mobileNavStack, setMobileNavStack] = useState<MobileNavLevel[]>(() => [
    { title: 'Products', level: 'categories', data: initialData },
  ]);
  const [direction, setDirection] = useState(1);

  useEffect(() => {
    if (initialData.length > 0) {
      setActiveCategory(initialData[0]);
      setMobileNavStack([{ title: 'Products', level: 'categories', data: initialData }]);
    }
  }, [location, initialData]);

  const handleMobileCategoryClick = useCallback((category: MenuCategory) => {
    setDirection(1);
    setMobileNavStack(stack => [
      ...stack,
      { title: category.name, level: 'products', data: category },
    ]);
  }, []);
  ...
  const handleCategoryHover = useCallback((category: MenuCategory) => {
    ...
    >
                ...
                {activeCategory.description && (
                  <div className="pb-4 border-b border-gray-200 dark:border-gray-700 mb-4">
                    ...
                    <Link
                      onClick={onClose}
                      to="/products"
                      className="text-sm font-semibold text-fuchsia-600 dark:text-fuchsia-400 hover:underline mt-1 inline-block"
                    >
                  ...
                  {activeCategory.items.map(item => (
                    <li key={item.name}>
                      <Link
                        onClick={onClose}
                        to={item.path}
                        className="group block p-3 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                      >
      ...
      <div className="lg:hidden flex flex-col h-full">
        <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b dark:border-gray-700">
          ...
          <h2 className="font-bold text-lg text-gray-900 dark:text-white text-center">
